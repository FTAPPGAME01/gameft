...
const resetGame = () => {
  ...
  socket.on('takeFicha', (data) => {
    console.log('Ficha tomada por:', data.player);
    const { rowId, index, player, emoji, type, points } = data;
    ...
    if (rowArray && rowArray[index] && rowArray[index].available) {
        // Marcar la ficha como no disponible
        rowArray[index].available = false;
        rowArray[index].emoji = emoji || '<img src="./assents/ftaoo.png" class="emoji-image">';
        ...
        if (!gameState.takenRowsByPlayer[player].includes(rowId)) {
            gameState.takenRowsByPlayer[player].push(rowId);
        }
        
        // Actualizar el contador de fichas tomadas
        gameState.takenCount++;
        
        // Actualizar el puntaje del jugador
        if (points) {
            gameState.score[player] = Math.max(0, (gameState.score[player] || 60000) + points);
        }
...
// Responder a pings de estado
socket.on('pingStatus', () => {
...
socket.on('playerJoined', (data) => {
    ...
    // Asegurarse de que el jugador tenga su entrada en takenRowsByPlayer
    if (!gameState.takenRowsByPlayer[username]) {
        gameState.takenRowsByPlayer[username] = [];
    }
    ...
    // Informar a todos sobre la actualización de la lista de jugadores
    io.emit('updatePlayersList', Object.keys(gameState.score).map(player => ({
        name: player,
        online: true
    })));
    
    // Enviar el estado actual del juego al jugador que se une
    socket.emit('initialState', gameState);
});
...
socket.on('resetGame', (data) => {
    // Verificar autorización (opcional)
    const canReset = data && data.pin === '25008';
    
    if (canReset || gameState.takenCount >= 16) {
        ...
        // Inicializar takenRowsByPlayer para cada jugador
        gameState.takenRowsByPlayer = {};
        Object.keys(currentScores).forEach(player => {
            gameState.takenRowsByPlayer[player] = [];
        });
...
// 3. Función para fusionar estados
const mergeGameStates = (currentState, newState) => {
// Asegurarse de que ambos estados son válidos
if (!currentState || !newState) return;

// Actualizar propiedades básicas
currentState.currentPlayer = newState.currentPlayer || currentState.currentPlayer;
currentState.timeLeft = newState.timeLeft || currentState.timeLeft;

// Fusionar puntajes sin perder información
if (newState.score) {
    Object.keys(newState.score).forEach(player => {
        if (!currentState.score[player]) {
            currentState.score[player] = newState.score[player];
        }
    });
}

// Actualizar estados de fichas solo si hay cambios
if (newState.diamondStates) currentState.diamondStates = newState.diamondStates;
if (newState.goldBarStates) currentState.goldBarStates = newState.goldBarStates;
if (newState.rubyStates) currentState.rubyStates = newState.rubyStates;
if (newState.trophyStates) currentState.trophyStates = newState.trophyStates;

// Fusionar información sobre fichas tomadas
if (newState.takenRowsByPlayer) {
    Object.keys(newState.takenRowsByPlayer).forEach(player => {
        if (!currentState.takenRowsByPlayer[player]) {
            currentState.takenRowsByPlayer[player] = [];
        }
        
        // Agregar filas tomadas sin duplicados
        newState.takenRowsByPlayer[player].forEach(row => {
            if (!currentState.takenRowsByPlayer[player].includes(row)) {
                currentState.takenRowsByPlayer[player].push(row);
            }
        });
    });
}

// Actualizar contador de fichas tomadas
let count = 0;
if (currentState.takenRowsByPlayer) {
    Object.values(currentState.takenRowsByPlayer).forEach(rows => {
        count += rows.length;
    });
}
currentState.takenCount = count;
};

// 4. Respaldo periódico automático del estado del juego
setInterval(() => {
console.log('Realizando respaldo periódico del estado del juego');
gameState.timestamp = Date.now();
saveGameStateToFile(gameState);
}, 5 * 60 * 1000); // Cada 5 minutos

// 5. Verificación periódica de la integridad del estado
setInterval(() => {
console.log('Verificando integridad del estado del juego');

// Verificar que todos los jugadores tienen sus entradas correspondientes
Object.keys(gameState.score).forEach(player => {
    if (!gameState.takenRowsByPlayer[player]) {
        gameState.takenRowsByPlayer[player] = [];
    }
});

// Recalcular el contador de fichas tomadas
let count = 0;
Object.values(gameState.takenRowsByPlayer).forEach(rows => {
    count += rows.length;
});

// Actualizar solo si hay discrepancia
if (gameState.takenCount !== count) {
    console.log(`Corrigiendo contador de fichas: ${gameState.takenCount} -> ${count}`);
    gameState.takenCount = count;
    gameState.timestamp = Date.now();
    saveGameStateToFile(gameState);
}
}, 15 * 60 * 1000); 
